...
(function () {
  ...
  var MIN_PASSWORD = 4;   // set-password mode me isse chhota password allowed nahi

  var appReady = false;

  /* Password localStorage me rehta hai (PROP_ADMIN_AUTH_V1).
     Pehli baar khaali mile to koi default seed nahi daalte — set-password mode chalta hai
     (naya password + confirm). Baad ke loads par sirf login prompt aata hai. */
  ...
  /* localStorage me password hai ya nahi — isi se first-run set-password mode tay hota hai */
  ...
  function initLock() {
    ...
    /* First run (localStorage me AUTH_KEY nahi) => set-password mode:
       naya password + confirm maango, match hone par hi save karo.
       Baad ke loads par sirf login prompt. */
    var firstRun = !hasStoredPassword();
    var confirmInput = null;

    if (firstRun) {
      confirmInput = document.createElement('input');
      confirmInput.className = 'input';
      confirmInput.id = 'lockConfirm';
      confirmInput.type = 'password';
      confirmInput.inputMode = 'text';
      confirmInput.autocomplete = 'new-password';
      confirmInput.placeholder = 'Password dobara likho';
      confirmInput.setAttribute('aria-label', 'Password dobara likho');
      confirmInput.required = true;
      if (input.parentNode) input.parentNode.insertBefore(confirmInput, input.nextSibling);

      input.setAttribute('autocomplete', 'new-password');
      input.placeholder = 'Naya password banao';
      if (title) title.textContent = 'Naya password banao';
      if (hint) hint.textContent = 'Pehli baar app khul raha hai — ek password set karo. Ye isi phone me save rahega.';
      if (submitBtn) submitBtn.textContent = 'Password save karo';
      if (forgot) forgot.hidden = true;   // pehli baar reset ka koi matlab nahi
    }
    ...
    /* set-password mode: naya password + confirm, match hone par hi save */
    function saveNewPassword() {
      var first = String(input.value || '');
      var again = String(confirmInput ? confirmInput.value : '');

      if (first.length < MIN_PASSWORD) {
        showError('Password kam se kam ' + MIN_PASSWORD + ' character ka rakho.');
        return false;
      }
      if (first !== again) {
        showError('Dono password match nahi kar rahe — dobara likho.');
        confirmInput.value = '';
        if (confirmInput.focus) confirmInput.focus();
        return false;
      }

      setStoredPassword(first);
      firstRun = false;
      return true;
    }
